  ...
  void handleChangePass() async {
    if (changePassFormKey.currentState!.validate()) {
      ...
      await user.updatePass(newPassword.text).then((value) {
        ...
        Get.snackbar(
            'Notification'.tr, 'Your password has change successfully.'.tr);
      });
    }
  }
  ...
  Future<void> handleRegister() async {
    ...
    if (registerFormGlobalKey.currentState!.validate()) {
      try {
        ...
        var user = await auth
            .signUp(email: registerEmail.text, password: registerPassword.text)
            .then((value) {
          Get.back();
          Get.snackbar(
              'Notification'.tr,
              'Register successfully. Check you email for confirmation mail.'
                  .tr);
        }); //print(res);
      } on AuthException catch (e) {
      ...
      try {
        ...
        await auth.recoveryPassword(forgotPassEmail.text).then((value) {
          ...
          Get.snackbar('Notification'.tr,
              'Please check your email for reset password mail.'.tr);
        });
      } catch (e) {
        print(e);
      } finally {
